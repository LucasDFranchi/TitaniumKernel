...
const TickType_t RECONNECT_INTERVAL_TICK = pdMS_TO_TICKS(15000);
...
static kernel_error_st write_to_file(void) {
    ...
    if (fputs(file_buffer, file) == EOF) {
        ...
        is_sd_card_present = false;  // add counter later
...
void sd_card_manager_loop(void* args) {
    ...
    while (1) {
        ...
        } else if (!is_sd_card_present && is_file_open) {
            close_and_dismount_sd_partition();
        } else {
            if ((xTaskGetTickCount() - last_wake_time) > RECONNECT_INTERVAL_TICK) {
                last_wake_time = xTaskGetTickCount();
                open_and_mount_sd_partition();
            }
        }
