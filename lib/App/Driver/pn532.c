...
bool init_PN532_I2C(uint8_t sda, uint8_t scl, uint8_t reset, uint8_t irq, i2c_port_t i2c_port_number) {
    ...
    i2c_config_t conf = {
        ...
        .sda_io_num       = SDA_PIN,   // Set to your SDA pin
        .scl_io_num       = SCL_PIN,   // Set to your SCL pin
        .sda_pullup_en    = GPIO_PULLUP_ENABLE,  // Enable pull-up for SDA
        .scl_pullup_en    = GPIO_PULLUP_ENABLE,  // Enable pull-up for SCL
        .master.clk_speed = 100000,  // Use 100 kHz or 400 kHz for initial testing
    };

    esp_err_t ret = ESP_ERROR_CHECK_WITHOUT_ABORT(i2c_param_config(PN532_I2C_PORT, &conf));
    ret += ESP_ERROR_CHECK_WITHOUT_ABORT(i2c_driver_install(PN532_I2C_PORT, conf.mode, 0, 0, 0));
    ret += ESP_ERROR_CHECK_WITHOUT_ABORT(i2c_set_timeout(PN532_I2C_PORT, 400000));
...
bool readdata(uint8_t *buff, uint8_t n) {
    ...
    if (i2c_master_cmd_begin(PN532_I2C_PORT, i2ccmd, pdMS_TO_TICKS(I2C_READ_TIMEOUT)) != ESP_OK) {
        // Reset i2c bus
        i2c_cmd_link_delete(i2ccmd);
        free(buffer);
        return false;
